...
<CardGroup cols={2}>
  <Card title="Block threats in real time" icon="shield-halved">
    Stop prompt injection, shell injection, PII exfiltration, and data leakage before they execute. 314/314 threats blocked. 100% forensic coverage.
  </Card>
  ...
  <Card title="Full audit trail" icon="file-lines">
    Every blocked call produces a tamper-evident record with a signed evidence hash, policy version, and decision trace ID. Built for GDPR, HIPAA, and PCI-DSS.
  </Card>
...
Spotlighting          ← trust-tags all content by source
    │
    ▼
Pre-Filter            ← fast path: <5ms, blocks obvious threats
    │
    ▼
